 ...
 * output: edge (YZW,c)
 */
kmer_t edge(kmer_t kmer, uint8_t c, uint8_t k){

	return (((kmer >> 3) | ((kmer & kmer_t(7))<<(3*k))) & (~kmer_t(7))) | c;

}

/*
 * input: edge (XYZ,W) stored as integer of 128 bits (see "format example" above)
 * output: just the kmer XYZ (stored in reverse order as ZYX as seen above)
 */
 ...
 * output: character Z
 */
uint8_t last_char(kmer_t kmer, uint8_t k){

	return (kmer >> (3*k)) & kmer_t(7);
...
string kmer_to_str(kmer_t kmer, int k){
	...
	char edge = toCHAR(kmer & kmer_t(7));
...
bool has_dollars(kmer_t kmer){

	return ((kmer >> 3) &  kmer_t(7)) == 0;
...
public:
	...
	dBg( const string & filename,
		...
		while (std::getline(file, str) and (nlines == 0 or read_lines < nlines)) { //getline reads header
			...
			assert(first_char!=0);
			kmer_t kmer = init_kmer(first_char);
		...
		for(uint64_t i = 1; i<=kmers.size();++i){
			...
			if(i == kmers.size() || get_kmer(kmers[i]) != get_kmer(prev_kmer)){
